...
public class GuiWinGame extends GuiScreen
{
    ...
    private void drawWinGameScreen(final int p_146575_1_, final int p_146575_2_, final float p_146575_3_) {
        ...
        f4 *= f4;
        f4 = f4 * 96.0f / 255.0f;
        worldrenderer.pos(0.0, this.height, GuiWinGame.zLevel).tex(0.0, f * f3).color(f4, f4, f4, 1.0f).endVertex();
        worldrenderer.pos(i, this.height, GuiWinGame.zLevel).tex(i * f3, f * f3).color(f4, f4, f4, 1.0f).endVertex();
        worldrenderer.pos(i, 0.0, GuiWinGame.zLevel).tex(i * f3, f2 * f3).color(f4, f4, f4, 1.0f).endVertex();
        worldrenderer.pos(0.0, 0.0, GuiWinGame.zLevel).tex(0.0, f2 * f3).color(f4, f4, f4, 1.0f).endVertex();
        tessellator.draw();
    }
    ...
    public void drawScreen(final int mouseX, final int mouseY, final float partialTicks) {
        ...
        Gui.drawTexturedModalRect(j, k, 0, 0, 155, 44);
        Gui.drawTexturedModalRect(j + 155, k, 0, 45, 155, 44);
        int l = k + 200;
        ...
        worldrenderer.begin(7, DefaultVertexFormats.POSITION_TEX_COLOR);
        worldrenderer.pos(0.0, k2, GuiWinGame.zLevel).tex(0.0, 1.0).color(1.0f, 1.0f, 1.0f, 1.0f).endVertex();
        worldrenderer.pos(j2, k2, GuiWinGame.zLevel).tex(1.0, 1.0).color(1.0f, 1.0f, 1.0f, 1.0f).endVertex();
        worldrenderer.pos(j2, 0.0, GuiWinGame.zLevel).tex(1.0, 0.0).color(1.0f, 1.0f, 1.0f, 1.0f).endVertex();
        worldrenderer.pos(0.0, 0.0, GuiWinGame.zLevel).tex(0.0, 0.0).color(1.0f, 1.0f, 1.0f, 1.0f).endVertex();
        tessellator.draw();
