...
public class Gui
{
    ...
    public void drawTexturedModalRect(final float xCoord, final float yCoord, final int minU, final int minV, final int maxU, final int maxV) {
        ...
        final WorldRenderer worldrenderer = tessellator.getWorldRenderer();
        worldrenderer.begin(7, DefaultVertexFormats.POSITION_TEX);
        worldrenderer.pos(xCoord + 0.0f, yCoord + maxV, Gui.zLevel).tex((minU + 0) * f, (minV + maxV) * f2).endVertex();
        worldrenderer.pos(xCoord + maxU, yCoord + maxV, Gui.zLevel).tex((minU + maxU) * f, (minV + maxV) * f2).endVertex();
        worldrenderer.pos(xCoord + maxU, yCoord + 0.0f, Gui.zLevel).tex((minU + maxU) * f, (minV + 0) * f2).endVertex();
        worldrenderer.pos(xCoord + 0.0f, yCoord + 0.0f, Gui.zLevel).tex((minU + 0) * f, (minV + 0) * f2).endVertex();
        tessellator.draw();
    }
