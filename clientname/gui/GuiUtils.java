...
public class GuiUtils extends Gui
{
    ...
    public static int drawString(final String text, final int x, final int y, final int color, final boolean shadow) {
        ...
        if (lines.length > 1) {
            ...
            for (int i = 0; i < lines.length; ++i) {
                j += GuiUtils.mc.fontRendererObj.drawString(lines[i], (float)x, (float)(y + i * (GuiUtils.mc.fontRendererObj.FONT_HEIGHT + 2)), color, shadow);
            }
            return j;
        }
        return GuiUtils.mc.fontRendererObj.drawString(text, (float)x, (float)y, color, shadow);
    }
    ...
    public static void drawChromaString(final String textIn, final int xIn, final int y, final boolean shadow) {
        ...
        for (int j = 0; j < textIn.split("\n").length; ++j) {
            ...
            for (int length = (charArray = textIn.split("\n")[j].toCharArray()).length, i = 0; i < length; ++i) {
                ...
                drawString(String.valueOf(c), x, y + j * (GuiUtils.mc.fontRendererObj.FONT_HEIGHT + 2), color, shadow);
                x += GuiUtils.mc.fontRendererObj.getStringWidth(String.valueOf(c));
            }
        }
    }
    
    public static int getChroma(final int x, final int y) {
        final long l = System.currentTimeMillis() - (x * 10 - y * 10);
    ...
    public static int drawCenteredString(final String text, final int x, final int y) {
        return drawString(text, x - GuiUtils.mc.fontRendererObj.getStringWidth(text) / 2, y);
    }
    
    public static int drawCenteredString(final String text, final int x, final int y, final int color) {
        return drawString(text, x - GuiUtils.mc.fontRendererObj.getStringWidth(text) / 2, y, color);
    }
    
    public static int drawCenteredString(final String text, final int x, final int y, final int color, final boolean shadow) {
        return drawString(text, x - GuiUtils.mc.fontRendererObj.getStringWidth(text) / 2, y, color, shadow);
    }
    ...
    public static int drawScaledCenteredString(final String text, final int x, final int y, final boolean shadow, final float scale) {
        return drawScaledString(text, x - GuiUtils.mc.fontRendererObj.getStringWidth(text) / 2, y, shadow, scale);
    }
    ...
    public static void drawCornerRoundedRect(final int paramInt1, final int paramInt2, final int paramInt3, final int paramInt4, final float radius, final int color) {
        final float f1 = (color >> 24 & 0xFF) / 255.0f;
        final float f2 = (color >> 16 & 0xFF) / 255.0f;
        final float f3 = (color >> 8 & 0xFF) / 255.0f;
        final float f4 = (color & 0xFF) / 255.0f;
        GlStateManager.color(f2, f3, f4, f1);
        drawRoundedCustomRect((float)paramInt1, (float)paramInt2, (float)paramInt3, (float)paramInt4, radius);
    }
    
    public static void drawCornerRoundedRectTwo(final int paramInt1, final int paramInt2, final int paramInt3, final int paramInt4, final float radius, final int color) {
        final float f1 = (color >> 24 & 0xFF) / 255.0f;
        final float f2 = (color >> 16 & 0xFF) / 255.0f;
        final float f3 = (color >> 8 & 0xFF) / 255.0f;
        final float f4 = (color & 0xFF) / 255.0f;
        GlStateManager.color(f2, f3, f4, f1);
        drawCustomTwoRect((float)paramInt1, (float)paramInt2, (float)paramInt3, (float)paramInt4, radius);
    }
    ...
    public static void drawRoundedCustomRect(final float paramFloat1, final float paramFloat2, final float paramFloat3, final float paramFloat4, final float paramFloat5) {
        final int i = 18;
        final float f1 = 90.0f / i;
        GlStateManager.disableTexture2D();
        GlStateManager.enableBlend();
        ...
        for (int j = 0; j <= i; ++j) {
            final float f4 = j * f1;
            GL11.glVertex2f((float)(f2 + paramFloat5 * Math.cos(Math.toRadians(f4))), (float)(f3 - paramFloat5 * Math.sin(Math.toRadians(f4))));
            GL11.glVertex2f(f2 + paramFloat5, (float)(f3 - paramFloat5 * Math.sin(Math.toRadians(f4))));
        }
        GL11.glEnd();
        GL11.glBegin(6);
        f2 = paramFloat1 + paramFloat5;
    ...
    public static void drawCustomTwoRect(final float paramFloat1, final float paramFloat2, final float paramFloat3, final float paramFloat4, final float paramFloat5) {
        final int i = 18;
        final float f1 = 90.0f / i;
        GlStateManager.disableTexture2D();
        GlStateManager.enableBlend();
